...
You can refer to elements of your decomposition:
```{r echo=TRUE}
almondTrend <- almond_dec$trend
almondSeason <- almond_dec$seasonal
```
This can be helpful to isolating seasonal variability in the data or examining drivers of random error that aren't explained by trends or seasonal fluctions in the data.
...
**Stationarity** is a condition of a time series where the mean and variance remains constant over time. Our trend and variability across seasons does not appear to fluctuate greatly across our time frame. We would want to apply caution in using some time series models and forecasting from a time series if our mean and variance was changing consistently over time.
...
## Conclusions
Time series analysis helped us better understand trends in ET in almond orchards. We observed strong, consistent seasonal patterns. We found that current ET was influenced by ET in previous 4 months, and were able to forecast data for missing months at the end of our time series. This can help with short term planning in addition to understanding the historical consumption of water. In your homework, you will examine trends in other types of agricultural fields.
