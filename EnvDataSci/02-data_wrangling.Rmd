...
`$`, `==`,`&`,`< >`,`|`,`plot`,`full_join`, `mean`,`max`,`sd`, `%>%`, `sumarise`, `filter`, `n`,`group_by`
...
The `filter` function in the `dplyr` package will create a new data frame based on your criteria. Let's filter out the data for the Peace River (gauge ID 2295637). 


You can use a **pipe** (`%>%`) to link data frames to a function and run functions in a sequences. Here we will pipe the `streamH` dataframe to `filter` so that column names can be easily referenced without repeating the dataframe name. In _tidyverse_ packages like `dplyr`, a pipe looks like this: `%>%`. Note that brand new versions of R offer a pipe in base R that looks a little different. It is too confusing to switch between the two. We will focus on the tidyverse version here. 

 Here we will pipe the `streamH` dataframe to `filter` so that column names can be easily referenced without repeating the dataframe name.
...
Chaining functions together can run multiple steps in the data wrangling process all at once and spare you from creating a lot of new data frames in your environment or frequently overwritting variable names. This is really key when you are working with big data and have to consider how much memory you are using in your data workflows.

<center>
```{r echo=FALSE, warning=FALSE,fig.width=8,fig.height=1.5}
par(bg = rgb(229/255,214/255,232/255), mai=c(0,0.2,0,7))
plot(load.image("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/photos/tutorial02/Leaf5.png"),   axes=FALSE)
a <- strwrap("How would you use the filter function to subset streamH to include all sites except Peace River?",width=70)
text(1300,75, "Check your understanding:", font=2, xpd=T, cex=1.25,adj = c(0,0))
for(i in 1:length(a)){
  text(1300,75+(i*400),paste(a[i]), xpd=T, cex=1.25,adj = c(0,0))
}
...
You can use the `group_by` and `summarise` function from dplyr to calculate the mean over a variable containing different grouping factors. In this case, that would not mean much since there is a random assortment of rivers with different baseline heights for each flood category. Instead it makes sense to look at each river individually. We will `pipes` to run the functions in sequence:
...
You can link other functions like filter, to grouping variables. For example, rather than thinking about average height, it may be helpful to characterize how long each gauge was in a certain flood category for each river. It is useful to subset the data frame  so that you can characterize the time spent under that condition.
