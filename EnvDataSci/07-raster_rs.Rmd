...
`rast`,`vect`, `project`, `plotRGB`,`extract`, _raster math_
...
```{r echo=FALSE}
terra::plotRGB(preR, r=3,g=2,b=1, stretch="lin")
```
...
```{r echo=FALSE}
plotRGB(postR, r=3,g=2,b=1, stretch="lin")
```
...
Sentinel 2 data is represented as a **digital number** where the actual reflectance is multiplied by 10,000 for each band. Since we are only working with sentinel data, we don't need to convert to an actual reflectance value (0-1 scale). Let's convert the values to reflectance:

```{r rasterCalc, echo=TRUE, fig.width=12}
# mathematical operations can be applied to a raster
preR <- pre/10000
postR <- post/10000
...
plotRGB(postR, r=3,g=2,b=1, # image and bands
        stretch="lin", # contrast stretch
        main="True color",
        axes=TRUE)
plotRGB(postR, r=4,g=3,b=2, # image and bands
        stretch="lin", # contrast stretch
        main="False color",
        axes=TRUE)
```

At any time you get get information about your raster by running the raster name in the console:
```{r echo=TRUE}
preR
```
There are key characteristics: coordinate reference system, extent, value ranges
...
northBoundv <- terra::vect("/cloud/project/activity07/north_bound.shp")

midBoundv <- terra::vect("/cloud/project/activity07/mid_bound.shp")

```

```{r vectorRead, echo=FALSE}
northBoundv <- terra::vect("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/data/chapter07/northBound.shp")

midBoundv <- terra::vect("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/data/chapter07/midBound.shp")
```

Let's examine the characteristics of the data:
```{r echo=TRUE}
northBoundv
```

There is a glaring issue with this data. The coordinate system is different from our raster. Plotting the two together means that we don't see the polygon on our map:
```{r plotBound, echo=TRUE}
plotRGB(postR, r=3,g=2,b=1, stretch="lin")
terra::plot(northBoundv, border="tomato3", fill=NA, lwd=3, add=TRUE)
```

The coordinate system will need to be **projected** to match that of the raster. When we are working with spatial analysis that is focused on analysis of area or distance, a projected coordinate system will be important. Projecting rasters is computationally intensive and we just want to conduct a pixel-wise analysis of the two rasters so we will reproject the two polygons. You can use the epsg code from the raster to do the projection:
```{r project, echo=TRUE}
northBound <- terra::project(northBoundv, "epsg:4326")
midBound <- terra::project(midBoundv, "epsg:4326")
```
...
```{r plotbounds, echo=TRUE}
plotRGB(postR, r=3,g=2,b=1, stretch="lin")
terra::plot(northBound, border="tomato3", fill=NA, lwd=3, add=TRUE)
terra::plot(midBound, border="royalblue4",fill=NA, lwd=3, add=TRUE)
```
...
```{r ndviExtract, echo=TRUE}
# return a vector of cell values from the first polygon
ndvi_North <- terra::extract(ndviDiff,northBound)
# return a vector of cell values from the first polygon
ndvi_Mid <- terra::extract(ndviDiff,midBound)
```
