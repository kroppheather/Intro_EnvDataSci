...
```{r eval=FALSE}
oct <- terra::rast("/cloud/project/activity08/Oct_12.tif")

```

```{r readO, echo=FALSE}
oct <- terra::rast("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/data/chapter08/Oct_12.tif")
...
```{r readR, echo=FALSE}
drStack <- terra::rast(c("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/data/chapter08/May_19.tif",
...
```{r plotstack, echo=TRUE}
terra::plot(drStack[[16:20]])
```

Finally, you will want to read in the points with known landcover classes. 

```{r eval=FALSE}
lc <- terra::vect("/cloud/project/activity08/land_pts.shp")
```

```{r dat, echo=FALSE}
lc <- terra::vect("/Users/hkropp/Documents/GitHub/Intro_EnvDataSci/EnvDataSci/data/chapter08/land_pts.shp")
```
```{r lcView, echo=TRUE}
head(values(lc))
```
...
# extract pixel data
train <- terra::extract(drStack, trainPts)
...
trainDF <- na.omit(cbind(y=as.factor(trainTable$LCID), train))
...
# turn into table
validDF_rf <- data.frame(y=valid_Table$LCID, rf=valid_rf)
...
0.4*0.4*freq(nn_prediction)[1,3]

```

You can also view the predictions side by side:
```{r predictPlot, echo=TRUE, fig.width=10}
par(mfrow=c(1,2))
